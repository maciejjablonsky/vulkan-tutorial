...
LveSwapChain::~LveSwapChain()
{
    ...
    {
        vkDestroySemaphore(
            device.device(), renderFinishedSemaphores[i], nullptr);
        vkDestroySemaphore(
            device.device(), imageAvailableSemaphores[i], nullptr);
                                            ...
                                            uint32_t* imageIndex)
{
    ...
    if (vkQueueSubmit(device.graphicsQueue(),
                      1,
                      &submitInfo,
...
VkExtent2D LveSwapChain::chooseSwapExtent(
